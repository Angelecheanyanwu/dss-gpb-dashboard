...
const CameraStream: React.FC<CameraStreamProps> = ({
  ...
  return (
    <div 
      className={cn(
        "relative group h-full w-full overflow-hidden rounded-xl bg-card border-2 transition-all duration-300",
        isActive ? "border-primary shadow-lg scale-[0.99]" : "border-border/40 hover:border-primary/60"
      )}
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-slate-50">
        <div className="text-slate-300 font-mono text-xs animate-pulse">
      ...
      {showPersons && selectedStream?.id && (
        <div className="absolute inset-0 bg-white/90 backdrop-blur-md p-4 flex flex-col gap-2 overflow-y-auto">
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Identified Personnel</h4>
            <button onClick={() => setShowPersons?.(false)} className="text-slate-400 hover:text-slate-900">
          ...
          {personsLoading ? (
            ...
            <div className="grid grid-cols-2 gap-2">
              {streamFaces.map((face, idx) => (
                <div key={idx} className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex flex-col gap-1 items-center">
                  <div className="h-8 w-8 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden">
                    <User className="h-5 w-5 text-slate-400" />
                  </div>
                  <span className="text-[8px] font-bold text-slate-900 truncate w-full text-center">{face.name || 'Unknown'}</span>
                  <span className="text-[6px] font-medium text-blue-600 capitalize">{face.confidence || '98% Match'}</span>
                </div>
              ))}
              {streamFaces.length === 0 && (
                <div className="col-span-2 text-center text-slate-400 text-[10px] py-4">
